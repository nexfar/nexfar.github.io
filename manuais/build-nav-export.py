...
.flow-steps{display:flex;align-items:center;justify-content:flex-start;overflow-x:auto;min-width:0;padding-bottom:2px}
...
.flow-steps{scrollbar-width:thin;scrollbar-color:#d1d5db transparent;scroll-behavior:smooth;-webkit-overflow-scrolling:touch}
.flow-steps::-webkit-scrollbar{height:6px}
.flow-steps::-webkit-scrollbar-track{background:transparent}
.flow-steps::-webkit-scrollbar-thumb{background:#d1d5db;border-radius:3px}
.flow-steps::-webkit-scrollbar-thumb:hover{background:#b9bec7}
...
(function(){
  ...
  if(fs){
    ...
    fs.addEventListener('pointerdown',function(e){
      ...
      down=true;moved=false;sx=e.clientX;sl=fs.scrollLeft;
      try{fs.setPointerCapture(e.pointerId);}catch(_){}
    });
    fs.addEventListener('pointermove',function(e){
      if(!down)return;
      var dx=e.clientX-sx;
      if(Math.abs(dx)>3){moved=true;fs.classList.add('dragging');}
      fs.scrollLeft=sl-dx;
    });
