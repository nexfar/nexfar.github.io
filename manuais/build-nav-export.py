...
/* barra de tabs: esconde scrollbar, fade nas bordas (sinaliza scroll) e drag */
.flow-steps{scrollbar-width:none;-ms-overflow-style:none;scroll-behavior:smooth;-webkit-overflow-scrolling:touch}
.flow-steps::-webkit-scrollbar{display:none}
.flow-steps.scrollable{cursor:grab}
.flow-steps.dragging{cursor:grabbing;scroll-behavior:auto;user-select:none}
.flow-steps a{-webkit-user-drag:none}
.flow-steps.dragging a{pointer-events:none}
.flow-steps[data-edge="right"]{-webkit-mask-image:linear-gradient(90deg,#000 calc(100% - 26px),transparent);mask-image:linear-gradient(90deg,#000 calc(100% - 26px),transparent)}
.flow-steps[data-edge="left"]{-webkit-mask-image:linear-gradient(90deg,transparent,#000 26px);mask-image:linear-gradient(90deg,transparent,#000 26px)}
.flow-steps[data-edge="both"]{-webkit-mask-image:linear-gradient(90deg,transparent,#000 26px,#000 calc(100% - 26px),transparent);mask-image:linear-gradient(90deg,transparent,#000 26px,#000 calc(100% - 26px),transparent)}
@media print{
...
@media(max-width:900px){
  ...
  /* o cabecalho reserva 316px a direita para o mockup que o -92px puxa para
     cima; com o mockup embaixo essa reserva so espremia o inicio da secao */
  .section-card>.section-header{padding-right:28px}
}
/* Tipografia por breakpoint. O NAVEXPORT redefine .header-title em 24px sem
   media query, o que achatava o passo 32/24 que os manuais tinham: o titulo
   ficava do mesmo tamanho em 320px e em 1280px. Escala fixa por faixa (o
   register e product; tipografia fluida com clamp nao serve aqui). */
...
(function(){
  /* indice de secoes: ativa a secao clicada NA HORA, trava o spy durante o scroll
     suave e mantem a tab ativa sempre visivel na barra horizontal (mobile) */
  var fs=document.querySelector('.flow-steps');
  ...
  var lock=false, lockT=null, current=null;
  function centerActive(step){
    if(!fs||!step)return;
    if(fs.scrollWidth<=fs.clientWidth+2)return;
    var sr=step.getBoundingClientRect(), cr=fs.getBoundingClientRect();
    var delta=(sr.left+sr.width/2)-(cr.left+cr.width/2);
    if(Math.abs(delta)>1)fs.scrollBy({left:delta,behavior:'smooth'});
  }
  ...
  map.forEach(function(m){m.step.addEventListener('click',function(){
    current=null; setActive(m.step);   // ativa imediatamente
  ...
  /* barra horizontal das tabs: fade nas bordas sinaliza scroll; drag com mouse
     (toque ja rola nativo) */
  function updateEdge(){
    if(!fs)return;
    var max=fs.scrollWidth-fs.clientWidth;
    if(max<=1){fs.classList.remove('scrollable');fs.removeAttribute('data-edge');return;}
    fs.classList.add('scrollable');
    var l=fs.scrollLeft;
  ...
  if(fs){
    ...
    fs.addEventListener('click',function(e){if(moved){e.preventDefault();e.stopPropagation();moved=false;}},true);
    fs.addEventListener('scroll',updateEdge,{passive:true});
